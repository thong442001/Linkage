...
import { checkOTP_phone, sendOTP_quenMatKhau_phone } from '../../rtk/API';
...
const CheckPhone = (props) => {
  ...
  const handleResendOTP = () => {
    ...
    setIsLoading(true);
    setError('');
    dispatch(sendOTP_quenMatKhau_phone({ phone }))
      .unwrap()
      .then((response) => {
        if (response.status) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.label2}>
        Chúng tôi đã gửi 1 mã xác nhận gồm 4 số đến số điện thoại{' '}
        <Text style={styles.emailText}>{phone}</Text>. Mã này dùng để đặt lại mật khẩu mới cho tài khoản của bạn.
      </Text>
