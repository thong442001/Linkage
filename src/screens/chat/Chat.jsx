...
import {getGroupID, getMessagesGroup} from '../../rtk/API';
  ...
  const uploadFile = async file => {
    try {
      ...
      data.append('file', {
        ...
        name:
          file.fileName ||
          (file.type.startsWith('video/') ? 'video.mp4' : 'image.png'),
      });
      ...
      await dispatch(getGroupID({ID_group: ID_group, token: token}))
        ...
        .then(response => {
          ...
          if (response.group.isPrivate == true) {
            ...
            // lấy danh sách thành viền gr
            if (response.group.members) {
              setGroupMembers(response.group.members);
              console.log('gr member:', groupMembers);
            } else {
              console.log('không có member');
            }
          }
        })
        .catch(error => {
...
//gr member 
  const handleInputChange = text => {
    ...
    if (text.endsWith('@')) {
      ...
      setFilteredMembers(groupMembers); // Hiển thị tất cả thành viên ban đầu
    } else {
  ...
  const renderMessage = (message) => {
    ...
    return (
      <Text style={[styles.messageText, message.content.includes('@') ? styles.highlightedMessage : null]}>
        {parts.map((part, index) => (
          part.startsWith('@') ? 
            <Text key={index} style={styles.taggedText}>{part} </Text> : 
  ...
  return (
    <View
      ...
      {/* <FlatList
            ...
            /> */}
      {groupName != null && groupAvatar != null && (
        <ChatHeader
          name={groupName}
          avatar={groupAvatar}
          onGoBack={goBack}
      ...
      {/* hiển thị danh sách member trong gr */}
      {showMentions && (
        <FlatList
          ...
          renderItem={({item}) => (
            <TouchableOpacity
              onPress={() => {
                setMessage(prev => prev + `${item.first_name} ${item.last_name} `);
                setShowMentions(false);
              }}
              style={styles.mentionItem}>
              <View style={styles.mentionItemContainer}>
                <Text style={styles.mentionText}>
                  <Text style={styles.taggedText}>{item.first_name} {item.last_name}</Text>
