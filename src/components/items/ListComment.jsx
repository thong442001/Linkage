...
const ListComment = memo(({ comment, onReply, onEdit, onDelete }) => {
  ...
  const userReaction = comment_reactions.find((reaction) => reaction.ID_user._id === me._id);
  ...
  // Hàm xử lý nhấn giữ bình luận để mở bottom sheet
                          ...
                          <TouchableOpacity>
                              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                  {comment_reactions.length > 0 && (
                                      <Text style={styles.reactionText}>{comment_reactions.length}</Text>
                                  )}
                                  {topReactions.map((reaction, index) => (
                                      <View key={index}>
                                          <Text style={styles.reactionText2}>{reaction.ID_reaction.icon}</Text>
                                      </View>
                                  ))}
                              </View>
                          </TouchableOpacity>
