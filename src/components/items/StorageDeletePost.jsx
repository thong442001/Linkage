...
const handDeletePost = async () => {
    try {
        // Ẩn bài viết ngay lập tức


        // console.log('Xóa bài vĩnh viễn:', post._id);
        
        await dispatch(deletePost({ _id: post._id }))
            .unwrap()
            .then((response) => {
                console.log("Xóa vĩnh viễn thành công", response);
                setIsHidden(true); // Ẩn bài viết sau khi xóa thành công
            })
            .catch(error => {
                console.log("Lỗi khi xóa bài:", error);
                setIsDeleted(false); // Nếu lỗi, hiển thị lại bài viết
            });
    } catch (error) {
...
if (isDeleted) {
    return null; // Không render bài viết nếu đã bị ẩn
  }
            ...
            <View style={styles.header}>
             ...
             <TouchableOpacity
                      onPress={() =>
                        openBottomSheet(
                          25,
                          <TouchableOpacity onPress={() => {handDeletePost(), closeBottomSheet()}} style={styles.deleteButton}>
