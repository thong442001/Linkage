...
import {addPost, changeDestroyPost, deletePost} from '../../rtk/API';
...
const PostItem = ({post}) => {
  ...
  const changeDestroy = async () => {
    try {
      console.log('Xóa bài viết với ID:', post._id);
  
      await dispatch(changeDestroyPost({_id: post._id, _destroy: true}))
        .unwrap()
        .then(response => {
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          onPress={() =>
            openBottomSheet(
              25,
              <TouchableOpacity onPress={() =>{ changeDestroy(), closeBottomSheet()}}  style={styles.deleteButton}>
