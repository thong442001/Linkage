      ...
      try {
        const paramsAPI = {
          ...
          ID_post_shared: post.ID_post_shared
            ...
            : post._id, //nếu share bài post share thì share bài gốc
          tags: [],
        };
            ...
            <View>
              <View style={[styles.headerShare]}>
                <View style={styles.userInfo}>
                  <Image
                    source={{uri: post?.ID_user?.avatar}}
                    style={styles.avatar}
                  />
                  <View style={{marginLeft: width * 0.01}}>
                    <Text style={styles.name}>
                      {post?.ID_user?.first_name +
                        ' ' +
                        post?.ID_user?.last_name}
                    </Text>
            ...
            <View style={styles.header}>
              <View>
                {post.ID_post_shared ? (
                  <View style={styles.userInfo}>
                    <Image
                      source={{uri: post.ID_post_shared.ID_user.avatar}}
                      style={styles.avatar}
                    />
                    <View style={{marginLeft: width * 0.01}}>
                      <Text style={styles.name}>
                        {post.ID_post_shared.ID_user.first_name}{' '}
                        {post.ID_post_shared.ID_user.last_name}
                          ...
                          <Text>
                            ...
                            <Text
                              onPress={() =>
                                navigation.navigate('Profile', {
                                  _id: post.ID_post_shared.tags[0]._id,
                                })
                              }
                              style={[styles.name]}>
                            ...
                            {post.ID_post_shared.tags.length > 1 && (
                              <>
                                ...
                                <Text
                                  onPress={() =>
                                    console.log('Xem danh sách tag')
                                  }
                                  style={[styles.name]}>
                                  {post.ID_post_shared.tags.length - 1} người
                                  khác
                                </Text>
                              </>
                            )}
                  ...
                  <View style={styles.userInfo}>
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate('Profile', {_id: post.ID_user._id})
                      }>
                      <Image
                        source={{uri: post?.ID_user?.avatar}}
                        style={styles.avatar}
                        ...
                        {post.tags.length > 0 && (
                          <Text>
                            ...
                            <Text
                              ...
                              style={[styles.name]}>
                              {post.tags[0]?.first_name}{' '}
                              {post.tags[0]?.last_name}
                            </Text>
                            {post.tags.length > 1 && (
                              <>
                                <Text style={{color: 'gray'}}> và </Text>
        ...
        {!post._destroy && (
          <View style={styles.interactions}>
            <TouchableOpacity
              ...
              style={[
                styles.action,
                userReaction && {backgroundColor: 'white'},
              ]}
              ...
              <Text style={styles.actionText}>
                {userReaction ? (
                  ...
                  <Icon5 name="like2" size={20} color="black" />
                )}{' '}
                {/* Nếu đã react, hiển thị icon đó */}
              </Text>
              <Text
                style={[styles.actionText, userReaction && {color: '#0064E0'}]}>
                {userReaction
